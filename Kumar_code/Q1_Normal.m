...
w = ones(dim+1,1);
C=0;
fun = @(w)fun0(w,Gtemp,C);
gfun = @(w)gfun0(w,Gtemp,C);
Hvec = @()Hvec0(w,Gtemp,C);

%[w,f,gnorm] = SINewton(fun,gfun,Hvec,Y,w);
%Hard margin case no epsilon use init guess then fire ASM
[w,l,lcomp] = FindInitGuess(w,Atemp,btemp);
W=[];
fprintf('prev w = [%d,%d,%d], b = %d\n',w(1),w(2),w(3),w(4));
[w,lm] = ASM(w,gfun,Hvec,Atemp,btemp,W);
w=w(:,20001);
size(w)
fprintf('w = [%d,%d,%d], b = %d\n',w(1),w(2),w(3),w(4));
lcomp=lcomp(:,1);
